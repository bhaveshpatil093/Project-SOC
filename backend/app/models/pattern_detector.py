...
class PatternDetector:
    ...
    def match_pattern(self, pattern: AttackPattern, alerts: List[Dict[str, Any]]) -> Optional[PatternMatch]:
        matched_alerts = []
        explanation_parts = []
            ...
            for feat, required_val in step.required_features.items():
                ...
                if isinstance(required_val, str) and isinstance(alert_val, str):
                    if required_val.lower() not in alert_val.lower():
                        feature_ok = False
                        break
                elif required_val == True and not alert_val:
                    feature_ok = False
                    break
                elif required_val != alert_val:
                    feature_ok = False
                    break
