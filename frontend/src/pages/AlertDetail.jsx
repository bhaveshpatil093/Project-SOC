    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] p-6 shadow-lg flex flex-col md:flex-row gap-8 items-start md:items-center justify-between">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 flex-1 w-full text-center md:text-left">
          <ThreatGauge score={alert.threat_score * 100} size={isMobile ? 120 : 150} showLabel={false} />
          
          <div className="space-y-4 flex-1 w-full">
            <div className="flex flex-col md:flex-row items-center gap-3">
              <h1 className="text-xl md:text-2xl font-bold text-[var(--text_primary)] tracking-tight break-all md:break-normal">{alert.entity_key || "Unknown Entity"}</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div className="flex items-center gap-2 text-[var(--text_secondary)]">
                <Server className="h-4 w-4 text-[var(--text_secondary)]" />
                <span className="font-medium">{alert.host_id || "N/A"}</span>
              </div>
              <div className="flex items-center gap-2 text-[var(--text_secondary)]">
                <User className="h-4 w-4 text-[var(--text_secondary)]" />
                <span className="font-medium">{alert.user_name || "N/A"}</span>
              </div>
              <div className="flex items-center gap-2 text-[var(--text_secondary)]">
                <Terminal className="h-4 w-4 text-[var(--text_secondary)]" />
        ...
        <div className="flex flex-col sm:flex-row md:flex-col items-center md:items-end gap-4 w-full md:min-w-[200px] mt-4 md:mt-0">
          <div className="flex items-center gap-2 bg-[var(--bg_primary)] px-3 py-2 rounded-lg border border-[var(--border)] w-full">
            <span className="text-xs text-[var(--text_secondary)] font-medium">Status:</span>
            <select 
              value={alert.alert_status || "open"} 
              onChange={(e) => updateStatusMutation.mutate({ id: alert._id || alert.id, status: e.target.value })}
              className="flex-1 bg-transparent text-sm font-semibold text-[var(--text_primary)] focus:outline-none cursor-pointer"
            >
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-[var(--text_primary)] px-4 py-2.5 rounded-lg font-medium transition-colors shadow-lg shadow-purple-900/20"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] p-6 flex flex-col">
          <h3 className="text-lg font-bold text-[var(--text_primary)] mb-6 flex items-center gap-2">
          ...
          <div className="h-[250px] mb-6 overflow-x-auto hide-scrollbar">
            {shapData.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-[var(--text_secondary)] border border-dashed border-[var(--border)] rounded-lg">
                No feature importance data available.
              </div>
            )}
          </div>

          <blockquote className="border-l-4 border-blue-500 bg-[var(--bg_primary)]/50 p-4 rounded-r-lg text-[var(--text_secondary)] text-sm leading-relaxed italic">
        ...
        <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] p-6 flex flex-col">
          ...
          <div className="space-y-4 overflow-y-auto flex-1">
            {alert.mitre_technique_ids && alert.mitre_technique_ids.length > 0 ? (
                ...
                <div key={idx} className="bg-[var(--bg_primary)] border border-[var(--border)] rounded-lg p-4 hover:border-[var(--border)] transition-colors group">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[var(--text_primary)] font-medium mt-1">Matched Technique Signature</p>
                      <p className="text-xs text-[var(--text_secondary)] mt-1 capitalize">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="text-[var(--text_secondary)] hover:text-blue-400 transition-colors p-1 bg-[var(--bg_secondary)] rounded group-hover:bg-[var(--bg_tertiary)]"
              ...
              <div className="h-full flex items-center justify-center text-[var(--text_secondary)] border border-dashed border-[var(--border)] rounded-lg py-12">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-[var(--text_primary)]">Entity Timeline & Triggers</h3>
        </div>
        
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
          {/* Vertical Timeline */}
          <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] xl:col-span-1 h-full">
            <div className="p-4 md:p-6 flex items-center justify-between border-b border-[var(--border)] xl:border-none cursor-pointer xl:cursor-default" onClick={() => isMobile && setShowTimeline(!showTimeline)}>
              <h4 className="text-sm font-bold text-[var(--text_secondary)] uppercase tracking-wider">Event Sequence</h4>
              {isMobile && (
                <button className="text-[var(--text_secondary)]">
              ...
              {timeline.length > 0 ? timeline.map((tItem, idx) => {
                ...
                return (
                  <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow ${isCurrent ? 'bg-blue-500 border-blue-900 shadow-blue-500/50 z-10' : 'bg-[var(--bg_secondary)] border-[var(--border)] text-[var(--text_secondary)]'}`}>
                      {isCurrent ? <AlertTriangle className="h-4 w-4 text-[var(--text_primary)]" /> : <Clock className="h-4 w-4" />}
                    </div>
                    <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border ${isCurrent ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[var(--bg_primary)]/50 border-[var(--border)]'}`}>
                      <div className="flex items-center justify-between mb-1">
                        <Badge variant={tItem.threat_level}>{tItem.threat_level}</Badge>
                        <span className="text-xs text-[var(--text_secondary)] font-mono">{formatDate(tItem.timestamp)}</span>
                      </div>
                      <div className="text-sm text-[var(--text_secondary)] font-medium mt-2">{tItem.log_type}</div>
                ...
                <p className="text-[var(--text_secondary)] text-sm italic">No recent timeline events found for this entity.</p>
              )}
            </div>
            </div>
          )}
          ...
          <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] overflow-hidden xl:col-span-2">
            <div className="px-6 py-4 border-b border-[var(--border)]">
              <h4 className="text-sm font-bold text-[var(--text_secondary)] uppercase tracking-wider">Triggered Rules</h4>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-[var(--bg_primary)]/50 border-b border-[var(--border)]">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text_secondary)]">Rule ID</th>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text_secondary)]">Rule Details</th>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text_secondary)]">MITRE Technique</th>
                ...
                <tbody className="divide-y divide-[var(--border)]/50">
                  {alert.triggered_rules && alert.triggered_rules.length > 0 ? (
                    alert.triggered_rules.map((rule, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-[var(--bg_tertiary)]/50 transition-colors">
                          <td className="px-6 py-4 text-sm font-mono text-blue-400">{ruleId}</td>
                          <td className="px-6 py-4 text-sm text-[var(--text_secondary)]">{ruleName}</td>
                          <td className="px-6 py-4 text-xs font-mono text-[var(--text_secondary)]">{technique}</td>
                    ...
                    <tr>
                      <td colSpan="4" className="px-6 py-8 text-center text-[var(--text_secondary)]">
