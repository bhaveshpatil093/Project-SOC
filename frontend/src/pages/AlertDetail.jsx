...
export const AlertDetail = () => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 text-sm bg-[var(--bg-secondary)] hover:bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] px-3 py-1.5 rounded-lg transition-colors"
          >
      ...
      <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] p-6 shadow-lg flex flex-col md:flex-row gap-8 items-start md:items-center justify-between">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 flex-1 w-full text-center md:text-left">
          ...
          <div className="space-y-4 flex-1 w-full">
            <div className="flex flex-col md:flex-row items-center gap-3">
              <h1 className="text-xl md:text-2xl font-bold text-[var(--text-primary)] tracking-tight break-all md:break-normal">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-[var(--border)] mt-1">
              <Tag className="w-3.5 h-3.5 text-[var(--text-secondary)] flex-none" />
              {(alert?.tags || []).map((tag) => (
                <span
                  key={tag}
              ...
              {showTagInput ? (
                <div className="relative">
                  <input
                    ...
                    className="px-2 py-1 text-xs rounded border border-blue-500 bg-[var(--bg-primary)] text-[var(--text-primary)] outline-none w-36"
                  />
                  {tagInput && allTags?.length > 0 && (
                    <div className="absolute top-full left-0 mt-1 w-52 bg-[var(--bg-primary)] border border-[var(--border)] rounded-lg shadow-xl z-30 max-h-40 overflow-y-auto">
                      {allTags
                        ...
                        .map(t => (
                          ...
                          >
                            ...
                            <span className="text-[var(--text-secondary)] bg-[var(--bg-secondary)] px-1.5 rounded-full">{t.count}</span>
                          </div>
                        ))}
                    </div>
                  )}
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 rounded-full bg-[var(--bg-tertiary)] hover:bg-blue-500/20 text-xs text-[var(--text-secondary)] hover:text-blue-400 border border-dashed border-[var(--border)] hover:border-blue-500/40 transition-all"
                >
        ...
        <div className="flex flex-col sm:flex-row md:flex-col items-center md:items-end gap-4 w-full md:min-w-[200px] mt-4 md:mt-0">
          <div className="flex items-center gap-2 bg-[var(--bg-primary)] px-3 py-2 rounded-lg border border-[var(--border)] w-full">
            ...
            <select
              ...
              className="flex-1 bg-transparent text-sm font-semibold text-[var(--text-primary)] focus:outline-none cursor-pointer"
            >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-[var(--text-primary)] px-4 py-2.5 rounded-lg font-medium transition-colors shadow-lg shadow-purple-900/20"
          >
        ...
        <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] p-6 flex flex-col">
          ...
          <div className="h-[250px] mb-6 overflow-x-auto hide-scrollbar">
            {shapData.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-[var(--text-secondary)] border border-dashed border-[var(--border)] rounded-lg">
                No feature importance data available.
              </div>
            )}
          </div>

          <blockquote className="border-l-4 border-blue-500 bg-[var(--bg-primary)]/50 p-4 rounded-r-lg text-[var(--text-secondary)] text-sm leading-relaxed italic">
            "
          ...
          <div className="space-y-4 overflow-y-auto flex-1">
            {alert.mitre_technique_ids && alert.mitre_technique_ids.length > 0 ? (
              alert.mitre_technique_ids.map((technique, idx) => (
                <div
                  key={idx}
                  className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-lg p-4 hover:border-[var(--border)] transition-colors group"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-[var(--text-secondary)] hover:text-blue-400 transition-colors p-1 bg-[var(--bg-secondary)] rounded group-hover:bg-[var(--bg-tertiary)]"
              ...
              <div className="h-full flex items-center justify-center text-[var(--text-secondary)] border border-dashed border-[var(--border)] rounded-lg py-12">
      ...
      {(alert.temporal_context || alert.is_off_hours) && (
        <div className="bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border)] mb-6 mt-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="text-lg font-bold text-[var(--text-primary)] flex items-center gap-2">
          ...
          <div className="p-4 bg-[var(--bg-tertiary)] border border-[var(--border)] rounded-lg mb-4">
            <div className="whitespace-pre-wrap font-mono text-sm text-[var(--text-primary)]">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
          {/* Vertical Timeline */}
          <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] xl:col-span-1 h-full">
            <div
                  ...
                  {timeline.length > 0 ? (
                    timeline.map((tItem, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex items-center justify-center w-10 h-10 rounded-full border-4 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow ${isCurrent ? 'bg-blue-500 border-blue-900 shadow-blue-500/50 z-10' : 'bg-[var(--bg-secondary)] border-[var(--border)] text-[var(--text-secondary)]'}`}
                          >
                            {isCurrent ? (
                          ...
                          <div
                            className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border ${isCurrent ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[var(--bg-primary)]/50 border-[var(--border)]'}`}
                          >
          ...
          <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] overflow-hidden xl:col-span-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-[var(--bg-primary)]/50 border-b border-[var(--border)]">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)]">
                      Rule ID
                    </th>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)]">
                      Rule Details
