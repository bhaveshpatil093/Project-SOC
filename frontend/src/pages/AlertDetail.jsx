  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 text-sm bg-[var(--bg_secondary)] hover:bg-[var(--bg_tertiary)] border border-[var(--border)] text-[var(--text_primary)] px-3 py-1.5 rounded-lg transition-colors"
          >
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 flex-1 w-full text-center md:text-left">
          ...
          <div className="space-y-4 flex-1 w-full">
            <div className="flex flex-col md:flex-row items-center gap-3">
              <h1 className="text-xl md:text-2xl font-bold text-[var(--text_primary)] tracking-tight break-all md:break-normal">
          ...
          <div className="space-y-4 overflow-y-auto flex-1">
            {alert.mitre_technique_ids && alert.mitre_technique_ids.length > 0 ? (
              alert.mitre_technique_ids.map((technique, idx) => (
                <div
                  key={idx}
                  className="bg-[var(--bg_primary)] border border-[var(--border)] rounded-lg p-4 hover:border-[var(--border)] transition-colors group"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-sm font-bold text-red-400 font-mono">{technique}</span>
                      <p className="text-[var(--text_primary)] font-medium mt-1">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-[var(--text_primary)]">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
          ...
          <div className="bg-[var(--bg_secondary)] rounded-xl border border-[var(--border)] xl:col-span-1 h-full">
            ...
            >
              <h4 className="text-sm font-bold text-[var(--text_secondary)] uppercase tracking-wider">
                Event Sequence
              </h4>
              {isMobile && (
                  ...
                  {timeline.length > 0 ? (
                    timeline.map((tItem, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex items-center justify-center w-10 h-10 rounded-full border-4 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow ${isCurrent ? 'bg-blue-500 border-blue-900 shadow-blue-500/50 z-10' : 'bg-[var(--bg_secondary)] border-[var(--border)] text-[var(--text_secondary)]'}`}
                          >
                            {isCurrent ? (
                              <AlertTriangle className="h-4 w-4 text-[var(--text_primary)]" />
                            ) : (
                              <Clock className="h-4 w-4" />
                            )}
                          </div>
                          <div
                            className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border ${isCurrent ? 'bg-blue-900/20 border-blue-500/50' : 'bg-[var(--bg_primary)]/50 border-[var(--border)]'}`}
                          >
                            <div className="flex items-center justify-between mb-1">
                              <Badge variant={tItem.threat_level}>{tItem.threat_level}</Badge>
                              <span className="text-xs text-[var(--text_secondary)] font-mono">
                                {formatDate(tItem.timestamp)}
                              </span>
                            </div>
                            <div className="text-sm text-[var(--text_secondary)] font-medium mt-2">
                              {tItem.log_type}
                            </div>
                            <Link
                    ...
                    <p className="text-[var(--text_secondary)] text-sm italic">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-[var(--bg_primary)]/50 border-b border-[var(--border)]">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text_secondary)]">
                      Rule ID
                    </th>
                    <th className="px-6 py-3 text-xs font-semibold text-[var(--text_secondary)]">
                      Rule Details
                  ...
                  {alert.triggered_rules && alert.triggered_rules.length > 0 ? (
                    alert.triggered_rules.map((rule, idx) => {
                      ...
                      return (
                        <tr
                          key={idx}
                          className="hover:bg-[var(--bg_tertiary)]/50 transition-colors"
                        >
                          <td className="px-6 py-4 text-sm font-mono text-blue-400">{ruleId}</td>
                          <td className="px-6 py-4 text-sm text-[var(--text_secondary)]">
                            {ruleName}
                          </td>
                          <td className="px-6 py-4 text-xs font-mono text-[var(--text_secondary)]">
                            {technique}
                          </td>
