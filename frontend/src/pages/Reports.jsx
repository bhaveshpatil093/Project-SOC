      ...
      <div className="bg-[var(--bg\_primary)] border border-[var(--border)] rounded-xl shadow-lg print:border-none print:shadow-none bg-white print:bg-white text-black font-sans max-w-5xl mx-auto overflow-hidden">
        <div className="p-8 pb-4 border-b border-gray-200 flex justify-between items-end print:pb-8">
          ...
          <div className="text-right text-sm">
            ...
            <p className="text-gray-900 font-mono">
              to {new Date(report.shift_end).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-red-50 p-3 rounded border border-red-100">
                ...
                <p className="text-2xl font-black text-red-700">
                  {report.critical_alerts + report.high_alerts}
                </p>
              </div>
              ...
              <div className="bg-purple-50 p-3 rounded border border-purple-100">
                ...
                <p className="text-2xl font-black text-purple-700">
                  {report.active_incidents.length}
                </p>
              </div>
            ...
            {/* Top Threats */}
            <div>
              <h3 className="text-xs font-bold text-gray-800 uppercase tracking-wider border-b border-gray-300 pb-1 mb-2 flex items-center gap-1">
                <ShieldAlert className="w-3 h-3" /> Top Threats
              </h3>
              <ul className="text-sm space-y-2">
                {report.top_threats.map((t, i) => (
                  <li key={i} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                    <span
                      className="font-medium text-blue-700 truncate w-32 cursor-pointer hover:underline"
                      onClick={() => window.open(`/alerts/${t.id || t._id}`, '_blank')}
                    >
                      {t.entity_key}
                    </span>
                    <span className="bg-red-100 text-red-800 px-1.5 py-0.5 rounded text-xs font-bold">
                      {Math.round(t.threat_score * 100)}
                    </span>
                  </li>
                ))}
              </ul>
            ...
            <div>
              ...
              <ul className="list-disc pl-5 space-y-2 text-[15px] text-gray-800">
                {report.recommendations.map((rec, idx) => (
                  <li key={idx} className="pl-1 text-blue-900">
                    {rec}
                  </li>
                ))}
              </ul>
        ...
        <div className="bg-gray-100 p-4 border-t border-gray-200 flex justify-end gap-4 print:hidden">
          <button
            ...
            className="flex items-center gap-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-800 px-4 py-2 rounded shadow-sm text-sm font-bold transition-colors"
          >
            <Copy className="w-4 h-4" /> Copy for Email
          </button>
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow-sm text-sm font-bold transition-colors"
          >
