...
const Workbench = () => {
  ...
  useKeyboardShortcuts({
    ...
    c: () => {
      if (selectedAlert)
        updateStatusMutation.mutate({ id: selectedAlert._id || selectedAlert.id, status: 'closed' })
    },
    '?': () =>
      alert(
  ...
  const avgTime =
    metrics.triagedToday > 0 ? Math.round(metrics.totalTime / metrics.triagedToday) : 0
  const tpRate =
    metrics.triagedToday > 0 ? Math.round((metrics.tpCount / metrics.triagedToday) * 100) : 0

  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col overflow-hidden bg-[var(--bg-primary)]">
      <div className="flex-1 flex overflow-hidden">
        {/* LEFT PANEL: Queue (30%) */}
        <div className="w-[30%] min-w-[350px] border-r border-[var(--border)] bg-[var(--bg-secondary)] flex flex-col">
          <div className="p-4 border-b border-[var(--border)] flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-bold text-[var(--text-primary)]">Triage Queue</h2>
              <Badge variant="info">{alerts.length} pending</Badge>
            </div>
            <button
              onClick={() => setShowUncertainOnly(!showUncertainOnly)}
              className={`flex items-center gap-2 text-sm px-3 py-1.5 rounded-lg border transition-colors ${showUncertainOnly ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-400' : 'bg-[var(--bg-primary)] border-[var(--border)] text-[var(--text-secondary)]'}`}
            >
              {showUncertainOnly ? (
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {isLoading ? (
              <div className="p-4 text-center text-[var(--text-secondary)]">Loading queue...</div>
            ) : alerts.length === 0 ? (
              <div className="p-8 text-center text-[var(--text-secondary)] border border-dashed border-[var(--border)] rounded-lg m-2">
              ...
              alerts.map((alert, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer p-3 rounded-lg border-l-4 border-y border-r transition-all ${levelColor} ${isSelected ? 'bg-[var(--bg-tertiary)] border-y-blue-500/50 border-r-blue-500/50 shadow-md scale-[1.02]' : 'bg-[var(--bg-primary)] border-y-[var(--border)] border-r-[var(--border)] hover:bg-[var(--bg-tertiary)]'}`}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className="font-mono text-sm font-bold text-[var(--text-primary)] truncate pr-2">
                        {alert.entity_key}
                      </span>
                      <span
                        className={`text-xs font-bold ${alert.threat_score > 0.8 ? 'text-red-400' : 'text-orange-400'}`}
                      >
        ...
        <div className="flex-1 flex flex-col bg-[var(--bg-primary)] overflow-y-auto">
          {selectedAlert ? (
            <div className="p-6 md:p-8 max-w-5xl mx-auto w-full flex-1 flex flex-col">
              {/* Header */}
              <div className="flex items-start gap-6 bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border)] shadow-sm mb-6">
                <ThreatGauge
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex gap-4 text-sm text-[var(--text-secondary)]">
                        ...
                        <span className="flex items-center gap-1">
                          <User className="h-4 w-4" /> {selectedAlert.user_name || 'Unknown'}
                        </span>
                      </div>
                    </div>
                    <Badge
                      variant={selectedAlert.threat_level}
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {selectedAlert.mitre_tactics?.map((t) => (
                      <span
                        key={t}
                        className="px-2 py-1 bg-red-900/20 text-red-400 border border-red-900/50 rounded text-xs font-mono"
                      >
                        {t}
                      </span>
                    ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 flex-1">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                  <div className="bg-[var(--bg-secondary)] p-6 rounded-xl border border-[var(--border)] flex-1">
                    ...
                    <div className="text-[var(--text-primary)] text-sm leading-relaxed bg-[var(--bg-primary)] p-4 rounded-lg border border-[var(--border)] h-full max-h-[300px] overflow-y-auto">
                ...
                <div className="flex flex-col gap-6">
                  {selectedAlert.consensus_level && (
                    <div className="bg-[var(--bg-secondary)] p-4 rounded-xl border border-[var(--border)]">
                      ...
                      <Badge
                        variant={
                          selectedAlert.consensus_level === 'strong'
                            ? 'critical'
                            : selectedAlert.consensus_level === 'split'
                              ? 'warning'
                              : 'info'
                        }
                  ...
                  <div className="bg-[var(--bg-secondary)] p-4 rounded-xl border border-[var(--border)] flex-1">
                    <h3 className="text-sm font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-4 flex items-center gap-2">
                    ...
                    <div className="space-y-3">
                      {selectedAlert.top_features?.slice(0, 5).map((f, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center text-sm p-2 bg-[var(--bg-primary)] rounded border border-[var(--border)]"
                        >
                          <span className="font-mono text-[var(--text-secondary)] truncate max-w-[150px]">
                            {typeof f === 'string' ? f : f.feature}
                          </span>
                          <span className="text-red-400 font-mono">
                            {(typeof f === 'string' ? 1 : f.value).toFixed(2)}
                          </span>
                        </div>
                      ))}
                      ...
                      {selectedAlert.top_features?.length > 0 && (
                        <div className="mt-4 p-3 bg-green-900/10 border border-green-500/30 rounded-lg">
                          <p className="text-xs text-green-400">
                            <span className="font-bold">Hint:</span> Would likely be scored Benign
                            if{' '}
                            <span className="font-mono">
                              {typeof selectedAlert.top_features[0] === 'string'
                                ? selectedAlert.top_features[0]
                                : selectedAlert.top_features[0].feature}
                            </span>{' '}
                            decreased by 30%.
                          </p>
                        </div>
              ...
              {/* Quick Action Bar */}
              <div className="bg-[var(--bg-secondary)] p-4 rounded-xl border border-[var(--border)] flex flex-wrap gap-4 items-center justify-between mt-auto shadow-lg">
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleTriageAction(false, selectedAlert)}
                ...
                <div className="flex gap-2 border-l border-[var(--border)] pl-4">
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-[var(--bg-primary)] hover:bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] rounded-lg text-sm transition-colors"
                  >
                    Investigate
                    <kbd className="ml-1 px-1.5 py-0.5 bg-[var(--bg-tertiary)] border border-[var(--border)] rounded text-xs text-[var(--text-secondary)]">
                      I
                    </kbd>
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/50 text-blue-400 rounded-lg text-sm transition-colors"
                  >
                    Skip
      ...
      <div className="h-10 bg-[var(--bg-tertiary)] border-t border-[var(--border)] flex items-center px-6 justify-between text-xs text-[var(--text-secondary)] font-mono">
        <div className="flex items-center gap-6">
          <span className="flex items-center gap-2">
            <CheckCircle className="h-3 w-3 text-green-500" />
            Triaged today:{' '}
            <strong className="text-[var(--text-primary)]">{metrics.triagedToday}</strong>
          </span>
          <span className="flex items-center gap-2">
            <Clock className="h-3 w-3 text-blue-500" />
            Avg triage time: <strong className="text-[var(--text-primary)]">{avgTime}s</strong>
          </span>
          <span className="flex items-center gap-2">
            <Activity className="h-3 w-3 text-orange-500" />
            TP Rate: <strong className="text-[var(--text-primary)]">{tpRate}%</strong>
          </span>
        </div>
