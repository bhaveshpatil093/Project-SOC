...
const IncidentListCard = ({ incident, isSelected, onClick }) => {
  ...
  const getStageBadge = (stage) => {
    const colors = {
      reconnaissance: 'bg-[var(--bg-tertiary)] text-[var(--text-secondary)]',
      ...
      multi_stage:
        'bg-gradient-to-r from-red-600 to-purple-600 text-[var(--text-primary)] border-none',
      unknown: 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]',
    }
  ...
  return (
    <div
      onClick={onClick}
      className={`p-4 border-b border-[var(--border)] cursor-pointer transition-all border-l-4 ${getBorderColor(incident.incident_threat_score)} ${isSelected ? 'bg-[var(--bg-secondary)]/80 shadow-inner' : 'hover:bg-[var(--bg-secondary)]/40'}`}
    >
      <div className="flex justify-between items-start mb-2">
        <h4 className="font-mono text-sm font-bold text-[var(--text-primary)] break-all">
          {incident.entity_key}
        </h4>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span
          className={`text-[10px] px-2 py-0.5 rounded uppercase font-bold ${incident.status === 'active' ? 'bg-blue-500/20 text-blue-400' : incident.status === 'escalated' ? 'bg-orange-500/20 text-orange-400' : 'bg-[var(--text-secondary)]/20 text-[var(--text-secondary)]'}`}
        >
          {incident.status}
        </span>
      </div>
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex flex-col items-center">
          <div className="text-[10px] text-[var(--text-secondary)] mb-1">MAX THREAT</div>
          <div className="relative w-10 h-10 rounded-full border-4 border-[var(--border)] flex items-center justify-center bg-[var(--bg-primary)]">
            <span
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {tactics.length === 0 && (
            <span className="text-xs text-[var(--text-secondary)]">None mapped</span>
          )}
        </div>
      </div>
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {techniques.map((t) => (
            <span
              key={t}
              className="text-xs bg-[var(--bg-secondary)] text-[var(--text-secondary)] px-2 py-1 rounded border border-[var(--border)] font-mono"
            >
              {t}
            </span>
          ))}
...
const IncidentDetailPanel = ({ incidentId }) => {
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-300">
      {/* Header */}
      <div className="p-6 border-b border-[var(--border)] bg-[var(--bg-primary)]/50 flex-shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-start gap-4">
          <div>
            ...
            <h2 className="text-2xl font-bold text-[var(--text-primary)] font-mono break-all">
              {detail.entity_key}
            </h2>
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 text-[var(--text-primary)] text-sm font-medium px-4 py-2 rounded-lg transition-colors shadow-lg flex items-center justify-center gap-2"
              >
                <Network className="h-4 w-4" /> Investigate with AI
              </button>
              <button
                onClick={() => generateIncidentReport(detail, detail.alerts)}
                className="bg-[var(--bg-secondary)] hover:bg-[var(--bg-tertiary)] border border-[var(--border)] text-[var(--text-primary)] text-sm font-medium px-4 py-2 rounded-lg transition-colors shadow-lg flex items-center justify-center gap-2"
        ...
        <div className="flex flex-wrap gap-6 mt-6 pt-4 border-t border-[var(--border)]/50">
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <span className="font-bold text-[var(--text-primary)]">{detail.alert_count}</span>{' '}
            Alerts
          </div>
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div>
          ...
          <div className="bg-[var(--bg-primary)] rounded-xl border border-[var(--border)] shadow-inner">
        ...
        {/* Timeline List */}
        <div>
          <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Event Timeline</h3>
          <div className="space-y-3">
            {detail.timeline.map((evt, idx) => (
              <div
                key={idx}
                className="flex gap-4 p-4 bg-[var(--bg-primary)]/50 rounded-lg border border-[var(--border)] hover:border-[var(--border)] transition-colors"
              >
                <div className="shrink-0 pt-0.5">
                  <div className="w-2 h-2 mt-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
                </div>
                <div>
                  <p className="text-sm text-[var(--text-secondary)] mb-1">{evt.event}</p>
...
export const Incidents = () => {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col gap-4">
      {/* Top Controls & Tabs */}
      <div className="flex items-center justify-between bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl p-3 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-[var(--bg-primary)] rounded-lg p-1 border border-[var(--border)]">
            <button
              ...
              className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'list' ? 'bg-blue-600 text-[var(--text-primary)] shadow' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
            >
              List View
            </button>
            <button
              onClick={() => setViewMode('graph')}
              className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'graph' ? 'bg-blue-600 text-[var(--text-primary)] shadow' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
            >
              Graph View
            </button>
          </div>
      ...
      {viewMode === 'graph' ? (
        <div className="flex-1 min-h-0 bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl shadow-xl overflow-hidden">
          <CorrelationGraph
            incidents={incidents}
            alerts={recentAlerts}
            onNodeClick={(d) => {
        ...
        <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-[35%] lg:w-[30%] flex flex-col bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl shadow-xl overflow-hidden shrink-0">
            <div className="p-4 border-b border-[var(--border)] bg-[var(--bg-primary)]/50 space-y-2">
              <div className="flex items-center gap-2 bg-[var(--bg-primary)] border border-[var(--border)] rounded-lg px-3 py-1.5">
                ...
                <select
                  ...
                  className="bg-transparent text-sm text-[var(--text-secondary)] focus:outline-none flex-1"
                >
          ...
          <div className="flex-1 bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl shadow-xl overflow-hidden hidden md:block">
            {selectedIncidentId ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-[var(--text-secondary)]">
                <Network className="h-16 w-16 text-[var(--text-primary)] mb-4" />
          ...
          {selectedIncidentId && (
            <div className="md:hidden fixed inset-0 z-50 bg-[var(--bg-primary)] flex flex-col">
              <div className="p-4 border-b border-[var(--border)] flex items-center gap-4 bg-[var(--bg-primary)]">
                <button
                  onClick={() => setSelectedIncidentId(null)}
                  className="p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-secondary)] rounded-lg"
                >
