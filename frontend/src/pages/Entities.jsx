...
const Entities = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex overflow-hidden">
      <div
        className={`flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 transition-all duration-300 ${selectedEntity ? 'pr-96' : ''}`}
      >
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] shadow-lg overflow-hidden flex flex-col">
                  ...
                  {isLoading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        <td className="px-6 py-4">
                          <div className="h-4 bg-[var(--bg-tertiary)] rounded w-4"></div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="h-4 bg-[var(--bg-tertiary)] rounded w-8"></div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="h-4 bg-[var(--bg-tertiary)] rounded w-12"></div>
                        </td>
                        <td className="px-6 py-4">
                    ...
                    entities.map((entity) => (
                      <tr
                        ...
                        className={`hover:bg-[var(--bg-tertiary)] transition-colors cursor-pointer ${selectedEntity?.entity_key === entity.entity_key ? 'bg-[var(--bg-tertiary)] border-l-2 border-l-indigo-500' : 'border-l-2 border-l-transparent'}`}
                      >
                        <td className="px-6 py-4">
                          <button
                            onClick={(e) => handleWatchToggle(e, entity)}
                            className="focus:outline-none"
                          >
                            <Star
                              className={`h-5 w-5 ${entity.is_watchlisted ? 'text-yellow-400 fill-yellow-400' : 'text-[var(--text-secondary)] hover:text-yellow-400'}`}
                            />
                          </button>
                        </td>
                        <td className="px-6 py-4">
      ...
      {/* Slide-out Panel */}
      <div
        className={`fixed inset-y-0 right-0 w-96 bg-[var(--bg-secondary)] border-l border-[var(--border)] shadow-2xl transform transition-transform duration-300 ease-in-out z-20 ${selectedEntity ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {selectedEntity && (
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[var(--bg-primary)] p-4 rounded-lg border border-[var(--border)]">
                  <div className="text-xs text-[var(--text-secondary)] mb-1">Peak Risk Score</div>
                  <div className="text-2xl font-bold text-[var(--text-primary)]">
                    {selectedEntity.peak_risk_score.toFixed(1)}
                  </div>
                </div>
                <div className="bg-[var(--bg-primary)] p-4 rounded-lg border border-[var(--border)]">
                  <div className="text-xs text-[var(--text-secondary)] mb-1">Total Incidents</div>
                  <div className="text-2xl font-bold text-[var(--text-primary)]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedEntity.top_tactics?.map((t) => (
                    <span
                      key={t}
                      className="px-2 py-1 text-xs bg-red-900/20 text-red-400 border border-red-900/50 rounded-md"
                    >
                      {t}
                    </span>
                  ))}
                ...
                {trends ? (
                  <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-lg p-4">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <div className="text-xs text-[var(--text-secondary)]">7-Day Trend</div>
                        <div className="flex items-center gap-2 text-lg font-bold text-[var(--text-primary)]">
                          ...
                          ) : trends.trend_7d === 'decreasing' ? (
                            <>
                              <TrendingDown className="h-5 w-5 text-green-500" /> Decreasing
                            </>
                          ) : (
                            <>
                  ...
                  <div className="h-24 bg-[var(--bg-primary)] rounded-lg border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] text-sm">
                    Loading trends...
                  </div>
                )}
              </div>
