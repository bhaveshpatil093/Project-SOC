...
export const ToastProvider = ({ children }) => {
  ...
  const showToast = useCallback(
    ({ id, title, message, level = 'info', duration = 5000, persistent = false }) => {
...
const ToastItem = ({ toast, onRemove }) => {
  ...
  return (
    <div
      className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border backdrop-blur-md shadow-2xl animate-in slide-in-from-right-8 fade-in duration-300 ${bgColors[toast.level] || bgColors.info} ${borderColors[toast.level] || borderColors.info}`}
    >
      <div className="shrink-0 mt-0.5">{icons[toast.level] || icons.info}</div>
      <div className="flex-1 min-w-0">
        <h4 className="text-sm font-bold text-[var(--text-primary)] tracking-wide truncate">
          {toast.title}
        </h4>
        {toast.message && (
      ...
      <button
        ...
        className="shrink-0 p-1 rounded-md text-[var(--text-secondary)] hover:bg-white/10 hover:text-[var(--text-primary)] transition-colors"
      >
        <X className="h-4 w-4" />
      </button>
    </div>
