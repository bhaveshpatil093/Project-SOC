...
export const BackupsPanel = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[var(--bg_secondary)] p-6 rounded-xl border border-[var(--border)]">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text_primary)]">Elasticsearch Snapshots</h3>
      ...
      <div className="bg-[var(--bg_secondary)] border border-[var(--border)] rounded-xl overflow-hidden">
        {isLoading ? (
          <div className="p-12 flex justify-center"><LoadingSpinner size="lg" /></div>
        ) : !backups || backups.length === 0 ? (
          <div className="p-12 text-center text-[var(--text_secondary)]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--bg_tertiary)]">
                <th className="px-6 py-4 text-xs font-semibold text-[var(--text_secondary)] uppercase tracking-wider">Snapshot Name</th>
                <th className="px-6 py-4 text-xs font-semibold text-[var(--text_secondary)] uppercase tracking-wider">Status</th>
                <th className="px-6 py-4 text-xs font-semibold text-[var(--text_secondary)] uppercase tracking-wider">Created</th>
                <th className="px-6 py-4 text-xs font-semibold text-[var(--text_secondary)] uppercase tracking-wider">Duration</th>
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {backups.map((snap) => (
                <tr key={snap.snapshot_name} className="hover:bg-[var(--bg_tertiary)]/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-[var(--text_primary)]">{snap.snapshot_name}</div>
                    <div className="text-xs text-[var(--text_secondary)] mt-1">{snap.indices.length} indices</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-[var(--bg_tertiary)] hover:bg-orange-500/20 text-orange-400 rounded transition-colors"
                    >
                      <Download className="h-3.5 w-3.5" /> Restore
                    </button>
                    <button
                      onClick={() => setDeleteSnap(snap.snapshot_name)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-[var(--bg_tertiary)] hover:bg-red-500/20 text-red-400 rounded transition-colors"
                    >
