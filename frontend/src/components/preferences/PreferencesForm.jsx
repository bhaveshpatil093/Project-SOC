...
import { Moon, Sun, Volume2, VolumeX, Bell, BellOff, RefreshCw, Eye, User, Save, RotateCcw } from "lucide-react";
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      <div className="flex justify-between items-center bg-[var(--bg_secondary)] p-6 rounded-xl border border-[var(--border)] shadow-sm">
        <div>
          ...
          <p className="text-sm text-[var(--text_secondary)] mt-1">Customize your workspace and notification thresholds.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[var(--bg_secondary)] p-6 rounded-xl border border-[var(--border)] shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Alerts Page Size</label>
              <select 
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Default Sort By</label>
              <select 
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Show Low Severity Alerts</label>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)] flex items-center gap-2">
                {store.notificationsEnabled ? <Bell className="h-4 w-4 text-green-500" /> : <BellOff className="h-4 w-4" />}
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Notify for Level</label>
              <select 
            ...
            <div className="flex justify-between items-center pt-4 border-t border-[var(--border)]/50">
              <label className="text-sm font-medium text-[var(--text_secondary)] flex items-center gap-2">
                {store.soundEnabled ? <Volume2 className="h-4 w-4 text-blue-500" /> : <VolumeX className="h-4 w-4" />}
                Sound Alerts
              </label>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Volume ({store.soundVolume}%)</label>
              <input 
                type="range" 
                min="0" 
                max="100" 
        ...
        <div className="bg-[var(--bg_secondary)] p-6 rounded-xl border border-[var(--border)] shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Dashboard Refresh (seconds)</label>
              <div className="flex items-center gap-3 w-1/2">
                <input 
                  ...
                  onChange={(e) => handleInputChange("dashboardRefreshInterval", parseInt(e.target.value))}
                  className="w-full accent-purple-500"
                />
                <span className="text-sm font-mono text-[var(--text_primary)]">{store.dashboardRefreshInterval}s</span>
              </div>
            </div>

            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Show Live Alert Stream</label>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="sr-only peer" 
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Default Time Range</label>
              <select 
            ...
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-[var(--text_secondary)]">Auto-load SLM Context</label>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="sr-only peer" 
              ...
              <label key={col} className="flex items-center gap-3 cursor-pointer">
                <div className="relative flex items-center">
                  ...
                  <div className="w-5 h-5 border-2 border-[var(--text_secondary)] rounded flex items-center justify-center peer-checked:bg-green-500 peer-checked:border-green-500 peer-disabled:opacity-50">
                    <svg className={`w-3 h-3 text-white ${isVisible ? 'block' : 'hidden'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                </div>
                <span className={`text-sm ${isVisible ? 'text-[var(--text_primary)]' : 'text-[var(--text_secondary)]'}`}>
