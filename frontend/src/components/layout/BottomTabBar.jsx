...
import { LayoutDashboard, ShieldAlert, MessageSquare, Menu, Network, Search, Activity, Settings, X } from 'lucide-react';
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--border)]/50">
          ...
          <button onClick={() => setIsMoreOpen(false)} className="p-1 rounded-full bg-[var(--bg_secondary)] text-[var(--text_secondary)] hover:text-[var(--text_primary)]">
            <X className="h-5 w-5" />
          </button>
        </div>
            ...
            return (
              <NavLink 
                ...
                className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${
                  isActive ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'bg-[var(--bg_secondary)]/50 text-[var(--text_secondary)] border border-[var(--border)]/50 active:bg-[var(--bg_tertiary)]'
                }`}
              >
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 h-16 bg-[var(--bg_primary)]/95 backdrop-blur-md border-t border-[var(--border)] z-50 flex items-center justify-around px-2 pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
          ...
          return (
            <NavLink 
              ...
              className={`flex flex-col items-center justify-center w-full h-full space-y-1 relative ${
                isActive ? 'text-blue-500' : 'text-[var(--text_secondary)] active:text-[var(--text_secondary)]'
              }`}
            >
              {isActive && <div className="absolute top-0 w-8 h-0.5 bg-blue-500 rounded-b-full shadow-[0_0_8px_rgba(59,130,246,0.8)]"></div>}
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center w-full h-full space-y-1 relative outline-none ${
            isMoreOpen ? 'text-[var(--text_primary)]' : 'text-[var(--text_secondary)] active:text-[var(--text_secondary)]'
          }`}
        >
          {isMoreOpen && <div className="absolute top-0 w-8 h-0.5 bg-slate-400 rounded-b-full"></div>}
