...
export const ThreatGauge = React.memo(({ 
  ...
  return (
    ...
    >
      <div style={{ width: size, height: size * 0.65 }} className="relative flex flex-col items-center justify-center">
        <svg 
          viewBox="0 0 200 120" 
      ...
      <div className="absolute bottom-0 flex flex-col items-center translate-y-3">
        <span className="font-bold text-[var(--text_primary)] leading-none" style={{ fontSize: size * 0.28 }}>
          {normalizedScore.toFixed(0)}
        </span>
        <span className="font-semibold tracking-wide mt-2" style={{ color, fontSize: size * 0.08 }}>
          {level}
        </span>
        {showLabel && (
          <span className="text-[var(--text_secondary)] font-medium uppercase tracking-wider mt-1" style={{ fontSize: size * 0.06 }}>
            {label}
          </span>
        )}
      </div>
