...
export const DataTable = ({ columns, data, loading, emptyMessage }) => {
  ...
  return (
    <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] overflow-hidden shadow-lg w-full">
          ...
          <thead className="bg-[var(--bg-primary)]/80 border-b border-[var(--border)]">
            <tr>
              {columns.map((col, idx) => (
                <th
                  key={idx}
                  className={`px-6 py-4 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider ${col.sortable ? 'cursor-pointer hover:text-[var(--text-primary)] select-none' : ''}`}
