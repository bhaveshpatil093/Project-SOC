...
export const Toast = ({ toast, onDismiss }) => {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-xl border backdrop-blur-md shadow-xl animate-in slide-in-from-bottom-8 fade-in duration-300 ${bgs[toast.type] || bgs.info}`}
    >
      {icons[toast.type] || icons.info}
      <p className="text-sm font-medium pr-4">{toast.message}</p>
      <button
        onClick={() => onDismiss(toast.id)}
        className="ml-auto opacity-60 hover:opacity-100 hover:bg-black/10 p-1 rounded transition-all"
      >
        <X className="h-4 w-4" />
      </button>
    </div>
