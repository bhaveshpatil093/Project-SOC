    ...
    useEffect(() => {
      ...
      const zoom = d3
        ...
        .on('zoom', (e) => {
          ...
          if (useCanvas)
            tick() // manually re-render canvas on zoom
            ...
            if (d.type === 'incident') {
              ...
              if (d.type === 'host') {
                ...
                if (d.score >= 0.8) {
                  ...
                  context.fillStyle = colors.high + '33'
                  context.strokeStyle = colors.high
                } else if (d.score >= 0.4) {
                  context.fillStyle = colors.medium + '33'
                  context.strokeStyle = colors.medium
                } else {
                  context.fillStyle = colors.low + '33'
                  context.strokeStyle = colors.low
                }
              } else {
                context.fillStyle = colors.bg_tertiary
                context.strokeStyle = colors.border
              }
              context.lineWidth = 2
            }
      ...
      <div className="relative w-full h-full bg-[var(--bg_primary)] border border-[var(--border)] rounded-xl overflow-hidden shadow-inner flex flex-col">
        ...
        <div className="absolute top-4 right-4 z-10 bg-[var(--bg_primary)]/80 backdrop-blur border border-[var(--border)] p-3 rounded-lg flex flex-col gap-3 shadow-lg">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-[var(--bg_secondary)] hover:bg-[var(--bg_tertiary)] text-xs text-[var(--text_primary)] px-3 py-1.5 rounded transition-colors border border-[var(--border)]"
          >
        ...
        {tooltip.show && (
          ...
          >
            ...
            <p className="font-mono text-blue-400 mb-1 break-all max-w-[200px]">
              {tooltip.content.id}
            </p>
            {tooltip.content.score !== undefined && (
              <p>Score: {(tooltip.content.score * 100).toFixed(0)}</p>
            )}
