...
export const EntityTimeline = ({
  ...
  const getLogIcon = (type) => {
    ...
    if (t.includes('network') || t.includes('syslog') || t.includes('firewall'))
  ...
  if (sortedAlerts.length === 0) {
    return (
      <div className="text-[var(--text_secondary)] text-sm italic p-4">
        ...
        {sortedAlerts.map((alert, idx) => {
          ...
          if (isCompact) {
            return (
              <div key={alert.id || idx} className="relative group">
                ...
                />
                <div
                  className={`flex items-center gap-3 text-sm ${isCurrent ? 'bg-[var(--bg_secondary)]/80 p-2 rounded-lg -ml-2 border border-[var(--border)]' : ''}`}
                >
                  <span className="text-[var(--text_secondary)] text-xs w-20 shrink-0">
                    {formatRelativeTime(alert.timestamp)}
                  </span>
                  <Badge variant={threatLevel}>{threatLevel}</Badge>
                  <span className="text-[var(--text_secondary)] truncate max-w-[200px]">
                    {alert.top_rule || alert.log_type || 'Unknown Rule'}
                  </span>
                  <Link
                    to={`/alerts/${alert.id || alert._id}`}
                    className="ml-auto text-blue-400 hover:text-blue-300 text-xs flex items-center"
                  >
          ...
          return (
            <div key={alert.id || idx} className="relative group">
              ...
              <div
                className={`ml-2 bg-[var(--bg_secondary)]/80 rounded-xl border ${isCurrent ? 'border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.15)]' : 'border-[var(--border)]'} p-4 transition-all hover:border-[var(--border)]`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4
                        className="text-sm font-bold text-[var(--text_primary)] truncate max-w-[250px]"
                  ...
                  <div className="flex items-center gap-3">
                    <Badge variant={alert.status === 'closed' ? 'benign' : 'tp'}>
                      {alert.status || 'open'}
                    </Badge>
                    <Badge variant={threatLevel}>{threatLevel}</Badge>
                  </div>
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  <div className="flex items-center gap-2 w-[120px]">
                    <span className="text-xs font-bold text-[var(--text_secondary)] w-6">
                      {score.toFixed(0)}
                    </span>
