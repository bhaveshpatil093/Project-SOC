...
export const LoadingSpinner = ({ size = "md", color = "blue", centered = true }) => {
  ...
  const spinner = (
    <div className={`animate-spin rounded-full border-t-transparent border-r-transparent ${borderClass} ${sizeClass}`}></div>
  );

  if (centered) {
    return <div className="flex justify-center items-center w-full h-full min-h-[100px]">{spinner}</div>;
  }
  return spinner;
};
