...
export const StatCard = ({
  ...
  const bgColors = {
    ...
    slate: 'bg-[var(--text-secondary)]/10 text-[var(--text-secondary)] border-[var(--border)]',
  }
  ...
  return (
    <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] p-6 flex items-center justify-between shadow-lg transition-transform hover:-translate-y-1 hover:border-[var(--border)]">
      <div>
        ...
        {subtitle && (
          <p className="text-xs text-[var(--text-secondary)] mt-2 font-medium">{subtitle}</p>
        )}
      </div>
      {Icon && (
        <div
